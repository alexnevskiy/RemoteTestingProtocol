...
- **ID** - восьмибитное поле, которое содержит уникальный идентификатор сообщения. Генерируется случайным образом. Ответ на сообщение должен быть с таким же идентификатором.
